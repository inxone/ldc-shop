...
- **商城体验**:
    ...
    - 📐 **商品规格（多规格）**: 同一商品可设置多个规格（如月付/季付/年付），不同规格独立价格与库存；首页显示价格区间与规格数；详情页规格选择器；后台与用户订单记录中显示规格标签；卡密按规格（商品）分别管理。
- **订单与发货**:
    - ✅ **支付回调验签**: 签名与金额校验。
...
- **管理后台**:
    - 📊 **销售统计**: 今日/本周/本月/总计。
    - ⚠️ **库存预警**: 低库存阈值配置与预警提示。
    - 🧩 **商品管理**: 新建/编辑/上下架/排序/限购；**可见范围**（所有人可见或仅某信任等级及以上）；**规格组 ID** 与 **规格标签**（多规格商品）；商品列表与订单列表展示规格信息。
